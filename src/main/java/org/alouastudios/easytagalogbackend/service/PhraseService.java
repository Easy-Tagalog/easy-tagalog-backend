...
public class PhraseService {
    ...
    public PhraseResponseDTO addPhrase(PhraseRequestDTO phraseRequest) {
        ...
        List<String> englishMeanings = getEnglishMeanings(phraseRequest.phraseWordRequestDTOS(), phraseWords);
    ...
    public PhraseResponseDTO updatePhrase(UUID uuid, PhraseRequestDTO phraseRequest) {
        ...
        List<String> foundEnglishMeanings = getEnglishMeanings(phraseRequest.phraseWordRequestDTOS(), foundWords);
    ...
    private List<String> getEnglishMeanings(List<PhraseWordRequestDTO> phraseWordRequestDTOs, Set<Word> phraseWords) {
        ...
        // Creating string list for the Phrase entity in the database
        List<String> phraseWordMeanings = new ArrayList<>();

        for (PhraseWordRequestDTO wordGuide : phraseWordRequestDTOs) {

            // First check if skip
            if (wordGuide.canSkip() != null && wordGuide.canSkip()) {
                phraseWordMeanings.add("<skip>");
                continue;
            }

            Word usedWord = phraseWordMap.get(wordGuide.wordUuid());

            // Adds english wordGuide of needed conjugation
            if (wordGuide.aspectMeaning() != null){
                phraseWordMeanings.add(wordGuide.aspectMeaning());
            }

            if (wordGuide.aspect() != null) {
                Conjugation usedConjugation = usedWord.getConjugation(wordGuide.tense());
                phraseWordMeanings.add(usedConjugation.getEnglish());
                continue;
            }

            // Else adds english wordGuide from the english list with matching uuid
            Translation usedTranslation = usedWord.getTranslation(wordGuide.partOfSpeech());
            phraseWordMeanings.add(usedTranslation);
        }

        return phraseWordMeanings;
    }
}
